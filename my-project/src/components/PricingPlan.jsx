  ...
  return (
    <section id="pricing">
        <div className="max-w-7xl mx-auto px-4 mt-20">
            ...
            <motion.div className='grid grid-cols-1 md:grid-cols-3 gap-8' initial="hidden" whileInView="visible" variants={{ visible: {transition: {straggerChildren: 0.5,}}}}>
                {PLANS_CONTENT.plans.map((plan,index)=>{
                    return(
                        <motion.div key={index} custom={index} variants={childVariant} className={`p-8 rounded-xl shadow-lg bg-neutral-950 ${plan.popular ? "border border-blue-900/80" : 'border border-neutral-800'}`}>
                            {plan.popular && (
